...
Agentic coding tools load these files directly into a model's context, where
they function as executable code, and skills bundle scripts the host actually
runs. They are distributed through unvetted channels — repos, community
marketplaces, gists — with no origin authentication, no integrity protection,
and no revocation. These specs define the formats that supply all three.

The formats are the product. Implementations are replaceable; what harness
vendors, marketplaces, and CI systems would standardize on is the wire format,
so it is specified independently of any one implementation.

## What a signature means here

Let's be precise about the goal, because it determines the whole design:
...
All six are normative. No schema identifier has been declared stable yet; see
...
- **T2 Impersonation and typosquatting.** `anthroplc/pdf-skill` published with
  the real `anthropic/pdf-skill`'s README copied verbatim. To someone skimming
  a marketplace listing the two are indistinguishable; only a verified signing
  identity separates them, and only if policy says that *name* requires that
  *identity*.
...
- **T4 Rollback and freeze.** A mirror keeps serving a correctly signed old
  version whose known vulnerability was fixed upstream. Every byte verifies —
  the attack is in version *selection*, not content.
- **T5 Partial-bundle attacks.** A signature covers only `SKILL.md` while the
  `scripts/` payload it instructs the agent to run is swapped, or an unlisted
  `helpers.py` is dropped next to the signed files.
...
Out of scope, and mitigated by other layers:
...
- **Runtime prompt injection** from web pages, tool output, or other content an
  agent pulls in mid-session. None of it was signed, so there is nothing to
...
- **D2 — Canonicalization is not optional.** Naive byte-hashing of Markdown
  ...
  schemes. Files the *host runs* are exempt: they are hashed as raw bytes,
...
- **D3 — Detached carriage, embedded fallback.** The signature lives beside the
  ...
  signature in YAML frontmatter — with strict validation and role gating,
  because the embedded region is excluded from the digest and is therefore the
  one place unsigned bytes could ride along. Specified in [03](03-bundle.md).
- **D4 — Keyless is the default; identity, not key possession, is the unit of
  ...
  theft and then reaching every consumer who already trusts that key — so in
  ...
  ephemeral key, the
  signature is recorded in a public transparency log, and the key is discarded.
  There is no long-lived secret to steal, the compromise window is minutes, and
  every signature under an identity is publicly visible. Organizations that
  cannot publish internal names to a public log bring their own X.509 chain and
  pin its root in policy instead. Specified in [05](05-keyless.md).
- **D5 — Verification without policy is meaningless.** Any attacker can validly
  ...
  the tripwire for T3 and for repo-transfer attacks. Specified in
  [04](04-policy.md).
- **D6 — Revocation is a feed, not a certificate status protocol.** A compact
  ...
  minutes anyway — what needs revoking is the identity or the artifact, not the
...
Two extension points are reserved but not yet specified: **attestations** —
separate signed statements over the same envelope and log (`promptsign/scan/v1`
for automated analysis, `promptsign/review/v1` for named human sign-off), which
is the honest answer to "is it malicious"; and **runtime action signing**, an
explicit non-goal for v1. [04](04-policy.md) reserves `require_attestations` for
the former.
...
2. For `scheme: keyless`, additionally performs the full offline algorithm of
...
signing material produced by other tools verifies.

## Versioning

The major version is carried **in the schema identifier**
...
(`…/v2`); it never redefines `v1`. Optional fields may be added within a
version — [04](04-policy.md) reserves several by name for exactly this reason.

Where an extension could otherwise be silently misread, the specs make it
visible instead: keyless bundles use `scheme: "keyless"` precisely so that a
verifier that only understands `ed25519` fails closed with "unsupported
signature scheme" rather than verifying against the embedded key without ever
checking the certificate.

## License

Apache License 2.0 — see [LICENSE](LICENSE). These specifications may be
implemented freely, by anyone, for any purpose, including commercially. An
independent implementation that interoperates is the point.
